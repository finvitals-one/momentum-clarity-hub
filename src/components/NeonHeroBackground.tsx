...
const NeonHeroBackground = () => {
  ...
  useEffect(() => {
    ...
    // Orange-themed synthwave shader
    const fragSrc = `
      ...
      vec3 sun(vec2 uv, float t) {
        ...
        vec3 sunColorTop = vec3(1.0, 0.85, 0.2);
        vec3 sunColorBot = vec3(1.0, 0.4, 0.05);
        ...
        vec3 glowCol = vec3(1.0, 0.5, 0.1) * glow1 +
                       vec3(1.0, 0.65, 0.2) * glow2 +
                       vec3(1.0, 0.9, 0.5) * glow3;
      ...
      vec3 sky(vec2 uv, float t) {
        ...
        vec3 colBot = vec3(0.5, 0.15, 0.02);
        vec3 colMid = vec3(0.15, 0.05, 0.02);
        vec3 colTop = vec3(0.03, 0.01, 0.02);
        ...
        col += vec3(1.0, 0.5, 0.1) * hazeStrength;
      ...
      vec3 grid(vec2 uv, float t) {
        ...
        vec3 gridColor = vec3(0.95, 0.45, 0.05);
        ...
        vec3 groundColor = vec3(0.03, 0.01, 0.01);
        col += groundColor;

        float fogBand = exp(-roadY * 10.0);
        col += vec3(0.5, 0.2, 0.05) * fogBand * 0.5;
      ...
      vec3 mountains(vec2 uv, float t) {
        ...
        float mountainHeight = (m1 + m2 + m3) * smoothstep(0.15, 0.5, au);

        float mountainTop = HORIZON + mountainHeight;
        float mountainMask = smoothstep(mountainTop + 0.005, mountainTop - 0.002, uv.y);

        vec3 col = vec3(0.02, 0.008, 0.005) * mountainMask;

        float edgeDist = abs(uv.y - mountainTop);
        float edgeGlow = exp(-edgeDist * 200.0) * smoothstep(0.15, 0.5, au);
        col += vec3(0.9, 0.4, 0.1) * edgeGlow * 0.3;
      ...
      vec3 sideGrid(vec2 uv, float t) {
        ...
        float fadeIn = smoothstep(0.0, 0.05, roadY);
        vec3 sideColor = vec3(0.6, 0.2, 0.02);
      ...
      vec3 sunReflection(vec2 uv, float t) {
        ...
        vec3 reflColor = mix(vec3(1.0, 0.6, 0.1), vec3(1.0, 0.35, 0.05), gradT);
      ...
      vec3 centerLine(vec2 uv, float t) {
        ...
        return vec3(1.0, 0.85, 0.5) * lineMask * dashPattern * distFade * 0.5;
      }
      ...
      vec3 roadEdges(vec2 uv, float t) {
        ...
        float edgeLine = smoothstep(0.002, 0.0, edgeDist);
      ...
      void main() {
        ...
        float hazeVal = noise(uv * 3.0 + vec2(t * 0.1, 0.0)) * 0.5 +
                        noise(uv * 7.0 + vec2(0.0, t * 0.15)) * 0.3 +
                        noise(uv * 15.0 + vec2(t * 0.05, t * 0.08)) * 0.2;
        col += vec3(0.5, 0.25, 0.05) * hazeVal * hazeMask * 0.15;
